...
import org.power.commons.data.JackSonUtils;
import org.power.commons.lang.util.CollectionUtils;
import org.power.commons.redis.config.HAClientConfig;
...
public class RedisClient implements RedisOperation {
     ...
     */
    public RedisClient(HAClientConfig clientConfig) {
    ...
    protected byte[] jsonSerialize(Object o) {
        byte[] res = null;
        try {
            res = JackSonUtils.toJson(o).getBytes("utf-8");
    ...
    protected Object jsonDeserialize(byte[] in, Class<?> cls) {
        ...
        Object res = null;
        try {
            res = JackSonUtils.json2Object(new String(in, "utf-8"), cls);
        } catch (UnsupportedEncodingException e) {
